...
function buildCommonPrompt(department, jobRole, companyType, experienceLevel, style) {
  ...
  p += `면접 질문 2개를 작성하세요. 반드시 물음표(?)로 끝나는 의문문이어야 합니다.\n\n`;
  p += `질문 1: 지원자의 관심 개발 분야나 커리어 목표를 묻는 질문.\n`;
  p += `질문 2: 팀 프로젝트에서 git 브랜치 전략, 코드 컨벤션, 역할 분담 등 구체적인 협업 방식을 묻는 질문.\n\n`;
  p += `규칙:\n`;
  p += `- 각 질문은 한 문장, 물음표(?)로 끝낼 것.\n`;
  p += `- 서술문이나 설명문 금지 — 반드시 의문문.\n`;
  p += `- 어미: "~셨나요?", "~있으신가요?", "~하셨습니까?", "~말씀해 주시겠어요?" 등.\n`;
...
function buildJobPrompt(department, jobRole, companyType, experienceLevel, style, interviewType, knowledgeEntries = [], previousQuestions = []) {
  ...
  if (interviewType === 'personality') {
    ...
    p += `${roleLabel} 직무 면접 인성 질문 3개를 작성하세요. 반드시 물음표(?)로 끝나는 의문문이어야 합니다.\n`;
    p += `${personalityDepthGuide}\n\n`;
    p += `질문 1: 팀 내 의견 충돌 또는 갈등 상황을 어떻게 해결했는지 묻는 질문.\n`;
    p += `질문 2: 예상치 못한 어려움이나 실패를 겪었을 때 어떻게 대처했는지 묻는 질문.\n`;
    p += `질문 3: 스스로 결정을 내리거나 방향을 바꿨던 경험을 묻는 질문.\n\n`;
    p += `규칙:\n`;
    p += `- 각 질문은 한 문장, 물음표(?)로 끝낼 것.\n`;
    p += `- 서술문·설명문 금지 — 반드시 의문문.\n`;
    p += `- 어미: "~셨나요?", "~있으신가요?", "~하셨습니까?" 등.\n`;
    p += `- 면접관 성격(${persona.tone}) 어투 유지.\n`;
    p += `- 반드시 아래 JSON 형식으로만 응답:\n`;
    p += `{"questions":["질문1?","질문2?","질문3?"]}`;
  } else if (experienceLevel === 'newcomer') {
    // 신입: CS 기초 + 시스템 설계 + 트렌드
    p += `${roleLabel} 분야 신입 지원자 면접 질문 3개를 작성하세요. 반드시 물음표(?)로 끝나는 의문문이어야 합니다.\n\n`;
    p += `질문 1: ${department} 관련 CS·전공 기초 개념을 묻는 질문.\n`;
    p += `(자료구조, 네트워크, 언어 특성, 프레임워크 차이 등 전공 기초 지식 확인)\n\n`;
    p += `질문 2: 간단한 프로그램이나 기능을 처음부터 어떻게 설계할지 접근 방법을 묻는 질문.\n`;
    p += `(로그인 흐름, 특정 프로그램 구조 설계 등 문제 해결 사고방식 확인)\n\n`;
    p += `질문 3: 생성형 AI·개발 도구 활용 경험이나 10년 뒤 개발 환경에 대한 생각을 묻는 열린 질문.\n\n`;
    p += `규칙:\n`;
    p += `- 각 질문은 한 문장, 물음표(?)로 끝낼 것.\n`;
    p += `- 서술문·설명문 금지 — 반드시 의문문.\n`;
    p += `- 신입이므로 깊은 실무 경험 전제 금지.\n`;
    p += `- 어미: "~인가요?", "~무엇인가요?", "~어떻게 하시겠어요?", "~말씀해 주시겠어요?" 등.\n`;
    p += `- 면접관 성격(${persona.tone}) 어투 유지.\n`;
    p += `- 반드시 아래 JSON 형식으로만 응답:\n`;
    p += `{"questions":["질문1?","질문2?","질문3?"]}`;
  } else {
...
function buildResumePrompt(department, jobRole, companyType, experienceLevel, style, resumeText, previousQuestions = []) {
  ...
  p += `위 이력서를 읽고, 이 사람에게만 할 수 있는 면접 질문 2개를 작성하세요. 반드시 물음표(?)로 끝나는 의문문이어야 합니다.\n`;
  p += `${depthGuide}\n`;
  if (previousQuestions.length > 0) {
  ...
  p += `질문 1: 이력서의 특정 프로젝트·수상·경험에서 담당 역할이나 기술적 판단을 묻는 질문.\n`;
  p += `질문 2: 이력서의 다른 경험에서 사용한 기술 스택 선택 이유 또는 아쉬웠던 점을 묻는 질문.\n\n`;
  p += `규칙:\n`;
  p += `- 각 질문은 한 문장, 물음표(?)로 끝낼 것.\n`;
  p += `- 서술문·설명문 금지 — 반드시 의문문.\n`;
  p += `- 이력서와 무관한 일반 질문 금지.\n`;
