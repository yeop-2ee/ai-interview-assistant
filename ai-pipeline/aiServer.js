...
function buildResumePrompt(department, jobRole, companyType, experienceLevel, style, resumeText, previousQuestions = []) {
  ...
  p += `질문 1: 이력서의 특정 경험에서 본인이 담당한 역할과 사용한 기술 스택을 묻는 질문. (프로젝트명 직접 언급 금지 — "프로젝트에서", "해당 경험에서" 등 간접 표현 사용)\n`;
  p += `질문 2: 이력서의 다른 경험에서 특정 기술·라이브러리·프레임워크를 선택한 이유 또는 구현 중 어려웠던 점을 묻는 질문. (프로젝트명 직접 언급 금지)\n\n`;
  p += commonQuestionRules(persona);
  p += `- 이력서와 무관한 일반 질문 금지.\n`;
  p += `- 반드시 아래 JSON 형식으로만 응답:\n`;
  p += `{"questions":["질문1?","질문2?"]}`;
