...
const DEFAULT_ANALYSIS_MODEL = 'llama3.1:8b';
...
const ANALYSIS_MODEL = process.env.ANALYSIS_MODEL || DEFAULT_ANALYSIS_MODEL;
...
app.post('/generate/questions', async (req, res) => {
  ...
  console.log(`[questions] 요청: ${department}/${jobRole} | 유형: ${interviewType} | 스타일: ${style} | 이력서: ${resumeText?.length || 0}자`);
  ...
  try {
    ...
    console.log(`[questions] 생성 완료 (${questions.length}개):\n` + questions.map((q,i) => `  ${i+1}. ${q}`).join('\n'));
...
app.post('/generate/followup', async (req, res) => {
  ...
  console.log(`[followup] 질문: "${question.substring(0,30)}..." | 답변: "${answer.substring(0,30)}..."`);
  ...
  try {
    ...
    console.log(`[followup] 결과: ${followup ? `"${followup}"` : 'null (생략)'}`);
    res.json({ followup });
  } catch (e) {
    console.error('[followup] 오류:', e.message);
...
app.post('/generate/summary', async (req, res) => {
  console.log('[summary] 요청 수신 — 더미 응답 반환 (분석 모델 미설치)');
  const send = setupSSE(res);
  send({ type: 'done', data: {
    name: '미상',
    oneLiner: '이력서가 업로드되었습니다.',
    skills: [],
    experience: '-',
    education: '-',
    projects: [],
