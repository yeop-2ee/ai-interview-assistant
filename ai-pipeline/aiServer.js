...
function buildCommonPrompt(department, jobRole, companyType, experienceLevel, style) {
  ...
  p += `질문 1 — [관심 분야 & 커리어 목표]\n`;
  p += `"어떤 분야 개발에 관심이 있나요?", "어떤 개발자가 되고 싶나요?", "앞으로 어떤 방향으로 성장하고 싶나요?" 유형.\n`;
  p += `"왜 지원했나요?" 같은 진부한 표현 금지. 구체적인 기술 분야나 목표를 끌어내는 질문.\n\n`;
  p += `질문 2 — [팀 협업 & 개발 환경 관리]\n`;
  p += `팀 프로젝트에서 역할 분담, 협업 방식, git/브랜치 전략, 코드 컨벤션 관리 등 실제 협업 경험을 묻는 질문.\n`;
  p += `"협업을 어떻게 하셨나요?"처럼 너무 포괄적인 표현 금지. 구체적인 상황이나 방식을 끌어내는 질문.\n\n`;
  p += `[작성 규칙]\n`;
  p += `- 질문은 짧고 명확하게. 한 문장, 물음표(?)는 끝에 하나만.\n`;
  p += `- 질문 안에 상황 설명이나 배경 서술 넣지 말 것 — 핵심 질문만 간결하게.\n`;
  p += `- 어미: "~셨나요?", "~있으신가요?", "~하셨습니까?", "~말씀해 주시겠어요?" 등 자연스러운 존댓말.\n`;
...
function buildJobPrompt(department, jobRole, companyType, experienceLevel, style, interviewType, knowledgeEntries = [], previousQuestions = []) {
  ...
  if (interviewType === 'personality') {
    ...
    p += `${roleLabel} 직무 면접에서 지원자의 업무 태도와 협업 방식을 검증하는 인성 질문 3개를 작성하세요.\n`;
    p += `${personalityDepthGuide}\n\n`;
    p += `질문 1 — 팀 내 의견 충돌 또는 우선순위 갈등 경험.\n`;
    p += `질문 2 — 예상치 못한 장애·일정 압박·실패 경험.\n`;
    p += `질문 3 — 스스로 결정을 주도하거나 방향을 바꾼 경험.\n\n`;
    p += `작성 규칙:\n`;
    p += `- 질문은 짧고 명확하게. 한 문장, 물음표(?)는 끝에 하나만.\n`;
    p += `- 질문 안에 상황 설명이나 배경 서술 넣지 말 것 — 핵심 질문만 간결하게.\n`;
    ...
    // 신입: 실제 면접 패턴 기반 — CS 기초 개념 + 시스템 설계 + 트렌드
    p += `${roleLabel} 분야 신입 지원자를 대상으로 한 면접 질문 3개를 작성하세요.\n`;
    p += `신입이므로 깊은 실무 경험 전제 금지. 기초 전공 지식과 사고 방식을 검증하세요.\n\n`;
    p += `질문 1 — [CS·전공 기초 개념]\n`;
    p += `자료구조(스택/큐/트리), 네트워크(HTTP 상태코드/REST), 운영체제, 언어 특성(인터프리터 vs 컴파일러 등), 프레임워크 차이(React vs Next.js, React vs NestJS 등) 중 ${department}에 맞는 기초 개념 질문.\n`;
    p += `"~가 무엇인지 아시나요?", "~의 차이점이 무엇인가요?" 형태도 허용.\n\n`;
    p += `질문 2 — [시스템 설계 & 문제 해결 접근]\n`;
    p += `간단한 프로그램이나 시스템을 처음부터 설계하는 방법을 묻는 질문. 예: "성적관리 프로그램을 만든다면 어떻게 접근하겠나요?", "로그인 기능의 전체 흐름을 설명해주세요."\n\n`;
    p += `질문 3 — [최신 기술 트렌드 & 도구 활용]\n`;
    p += `생성형 AI 사용 경험, AI 코딩 도구(GitHub Copilot, Cursor, Claude 등), 클라우드 서비스, "10년 뒤 개발 환경은 어떨 것 같나요?" 등 기술 트렌드에 대한 생각을 묻는 열린 질문.\n\n`;
    p += `작성 규칙:\n`;
    p += `- 질문은 짧고 명확하게. 한 문장, 물음표(?)는 끝에 하나만.\n`;
    p += `- 어미: "~인가요?", "~무엇인가요?", "~어떻게 하겠나요?", "~말씀해 주세요." 등 자연스러운 존댓말.\n`;
    p += `- 면접관 성격(${persona.tone}) 어투 유지.\n`;
    p += `- 반드시 아래 JSON 형식으로만 응답:\n`;
...
function buildResumePrompt(department, jobRole, companyType, experienceLevel, style, resumeText, previousQuestions = []) {
  ...
  p += `위 이력서를 읽고, 이 사람에게만 할 수 있는 질문 2개를 작성하세요.\n`;
  p += `${depthGuide}\n`;
  if (previousQuestions.length > 0) {
  ...
  p += `- 두 질문은 이력서의 서로 다른 경험(프로젝트, 수상, 활동 등)을 기반으로 할 것.\n`;
  p += `- 이력서에 수상 내역이나 특정 프로젝트가 있다면 "그 프로젝트에서 어떤 역할을 담당하셨나요?", "어떤 기술스택을 선택하셨고 그 이유는?" 같은 구체적 질문 적극 활용.\n`;
  p += `- 이력서에서 사용한 기술 스택이 있다면 해당 기술의 특징이나 선택 이유를 물어볼 것.\n`;
  p += `- 일반 기술 질문 금지 (예: "협업 어떻게 하나요?" 등 이력서와 무관한 질문)\n\n`;
  p += `질문 1 — 이력서에서 특정 프로젝트·수상·경험을 직접 언급하며 담당 역할이나 기술적 판단을 묻는 질문.\n`;
  p += `질문 2 — 질문 1과 다른 경험에서 사용한 기술 선택 이유 또는 아쉬웠던 점·개선 방향을 묻는 질문.\n\n`;
  p += `작성 규칙:\n`;
  p += `- 질문은 짧고 명확하게. 한 문장, 물음표(?)는 끝에 하나만.\n`;
  p += `- 질문 안에 상황 설명이나 배경 서술 넣지 말 것 — 핵심 질문만 간결하게.\n`;
  p += `- 이력서의 기술·경험을 질문에 자연스럽게 녹일 것. 프로젝트명·회사명 언급 금지.\n`;
  p += `- 어미: "~셨나요?", "~있으신가요?", "~하셨습니까?", "~말씀해 주시겠어요?" 등 자연스러운 존댓말.\n`;
  p += `- 면접관 성격(${persona.tone}) 어투 유지.\n`;
  p += `- 반드시 아래 JSON 형식으로만 응답:\n`;
