            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {PURPOSE_OPTIONS.map((opt) => (
                  <button
                    key={opt}
                    onClick={() => setPurpose((prev) => (prev === opt ? null : opt))}
                    className={`px-4 py-1.5 rounded-lg text-[12.5px] font-medium border transition-all ${
                      purpose === opt
                        ? "bg-[#4f52e8] text-white border-[#4f52e8] shadow-sm"
                ...
                {NATURALNESS_LEVELS.map(({ score, label }) => (
                  <button
                    ...
                    className={`flex-1 flex flex-col items-center gap-1 py-2.5 rounded-xl border text-center transition-all ${
                      naturalness === score
                        ? "bg-[#4f52e8] border-[#4f52e8] shadow-sm"
                        : "bg-white border-[#e4e7ef] hover:border-[#4f52e8]"
                    }`}
                  >
                    <span className={`text-[15px] font-bold ${naturalness === score ? "text-white" : "text-[#374151]"}`}>
                      {score}
                    </span>
                    <span className={`text-[9px] font-medium leading-tight whitespace-pre-line ${naturalness === score ? "text-white/80" : "text-[#9ca3af]"}`}>
                      {label}
                    </span>
                  </button>
                ))}
